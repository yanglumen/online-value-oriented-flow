...
import numpy as np
from models.flow_tf_forward_process import sample_weighted_interpolated_points
...
from evaluation.sequential_eval import parallel_d4rl_eval_transformer_version
...
class flow_tf_trainer():
    ...
    def __init__(
            self, argus, train_flow, behavior_flow, energy_model, dataset):
        self.argus = argus
        ...
        self.flow_tf_optimizer = torch.optim.Adam(self.model.parameters(), lr=argus.lr)
    ...
    def transformer_train(self, return_to_go, observations, actions, next_observations, next_actions):
        x_t, t, dx_dt, weights = sample_weighted_interpolated_points(
            argus=self.argus, observations=observations, actions=actions, energy=None, beta=self.argus.beta,
            energy_model=None, weighted_samples_type=WeightedSamplesType.linear_interpolation, clip_value=self.argus.x_t_clip_value,
        )
        pred_action = self.model(rtg=return_to_go, state=observations, action=x_t, t=t)
        loss = self.loss_fn(pred_action, dx_dt)
        self.flow_tf_optimizer.zero_grad()
        loss.backward()
        self.flow_tf_optimizer.step()
        return {"flow_transformer_loss":loss.detach().cpu().numpy().item()}
    ...
    def train(self, num_epochs, num_steps_per_epoch):
        try:
            ...
            epoch_offset = 0
        print(colored("flot mode train !!!!", "red"))
        for epoch in range(epoch_offset, num_epochs):
            ...
            for step in range(num_steps_per_epoch):
                ...
                if update_flow:
                    flow_loss_info = self.transformer_train(
                        return_to_go=batch.returns, observations=batch.observations, actions=batch.actions,
                        next_observations=None, next_actions=None
                    )
    ...
    def Q_guided_transformer_train(self, return_to_go, observations, actions, next_observations, next_actions):
        x_t, t, dx_dt, weights = sample_weighted_interpolated_points(
            argus=self.argus, observations=observations, actions=actions, energy=None, beta=self.argus.beta,
            energy_model=None, weighted_samples_type=WeightedSamplesType.linear_interpolation, clip_value=self.argus.x_t_clip_value,
        )
        pred_action = self.model(state=observations, action=x_t, t=t)
        flow_tf_loss = self.loss_fn(pred_action, dx_dt)
        Q_guided_loss = self.energy_model.get_scaled_q(obs=observations, act=pred_action, scale=self.argus.energy_scale)
        with torch.no_grad():
            Q_scale = (torch.abs(Q_guided_loss)).mean()
        Q_guided_loss = (torch.mean(Q_guided_loss / Q_scale.detach(), dim=1)).mean()
        loss = flow_tf_loss - self.argus.Q_guided_coef * Q_guided_loss
        self.flow_tf_optimizer.zero_grad()
        loss.backward()
        self.flow_tf_optimizer.step()
        return {"flow_transformer_loss":flow_tf_loss.detach().cpu().numpy().item(),
                "Q_guided_loss":Q_guided_loss.detach().cpu().numpy().item(),
                "Q_flot_loss":loss.detach().cpu().numpy().item()}

    def Q_value_guided_train(self, num_epochs, num_steps_per_epoch):
        try:
            self.load_critic_checkpoint(os.path.join(self.save_path, self.argus.mode, self.argus.domain, "_".join(self.env_name.split("-")), self.argus.load_critic_model))
            epoch_offset = 40
        except:
        ...
        for epoch in range(epoch_offset, num_epochs):
            ...
            for step in range(num_steps_per_epoch):
                ...
                if update_energy:
                    ...
                    loss.update(energy_loss_info)
                if update_flow:
                    flow_loss_info = self.Q_guided_transformer_train(
                        return_to_go=batch.returns, observations=batch.observations, actions=batch.actions,
                        next_observations=None, next_actions=None
                    )
                    loss.update(flow_loss_info)
    ...
    def eval(self):
        ...
        eval_results = parallel_d4rl_eval_transformer_version(
    ...
    def save_critic_checpoint(self):
        ...
        if self.argus.critic_type in [CriticType.iql, CriticType.ciql]:
            data.update({
                ...
                'flow_transformer': self.model.state_dict(),
            })
