...
class guided_flow_trainer():
    def __init__(self, argus, model, target_model, behavior_flow, energy_model, dataset):
        self.argus = argus
        self.model = model
    ...
    def flow_train(self, observations, actions):
        ...
        u = self.model(torch.cat([observations, x], dim=-1))
        ...
        with torch.no_grad():
            for t in np.linspace(time_start, time_end, steps):
                ...
                v = self.behavior_flow(torch.cat([observations, x], dim=-1), t_tensor)
                x = x + v * dt
        ...
        distillation_loss = self.loss_fn(u, x)
        pred_Q = self.energy_model.get_scaled_q(obs=observations, act=u, scale=1.0)
        loss = -pred_Q + distillation_loss
        loss = loss.mean()
        ...
        return {"flow_loss": loss.detach().cpu().numpy().item(),
                "distillation_loss": distillation_loss.mean().detach().cpu().numpy().item()}
        ...
        for epoch in range(epoch_offset, num_epochs):
            for step in range(num_steps_per_epoch):
                if self.argus.debug_mode:
                    ...
                    update_energy = True if epoch < 50 else False
                    update_flow = False if epoch < 50 else True
