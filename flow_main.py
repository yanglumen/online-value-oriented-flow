...
from trainer.flow_model_trainer import guided_flow_trainer
...
def import_parameters(var_kwargs=None, mode="guided_flow_parameters"):
    print(colored(f"Loading {mode} parameter ......", color="green"))
    if mode == "guided_flow_parameters":
        hyperparameters = guided_flow_parameters
...
def train(**var_kwargs):
    argus = dict2obj(import_parameters(var_kwargs, mode="guided_flow_parameters"))
    ...
    trainer = guided_flow_trainer(argus=argus, model=flow_model, energy_model=energy_model, dataset=dataset)
