...
from models.flow_model import FlowMatchingNet, OneStepFlowMatchingNet
...
from trainer.fql_trainer import guided_flow_trainer
...
def import_parameters(var_kwargs=None, mode="flow_q_learning_parameters"):
    print(colored(f"Loading {mode} parameter ......", color="green"))
    if mode == "flow_q_learning_parameters":
        hyperparameters = flow_q_learning_parameters
...
def train(**var_kwargs):
    argus = dict2obj(import_parameters(var_kwargs, mode="flow_q_learning_parameters"))
    ...
    train_flow = OneStepFlowMatchingNet(
        argus=argus, input_dim=argus.observation_dim + argus.action_dim, output_dim=argus.action_dim)
    target_train_flow = OneStepFlowMatchingNet(
        argus=argus, input_dim=argus.observation_dim + argus.action_dim, output_dim=argus.action_dim)
    ...
    trainer = guided_flow_trainer(
        argus=argus, model=train_flow, target_model=target_train_flow, behavior_flow=behavior_flow, energy_model=energy_model, dataset=dataset)
    trainer.train(num_epochs=200, num_steps_per_epoch=10000)
