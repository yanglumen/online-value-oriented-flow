...
class dist_flow_constrained_iql_critic(nn.Module):
    def __init__(self, args, sdim, adim):
        ...
        if args.rl_mode in [RLTrainMode.flow_constrained_rl2, RLTrainMode.dist_flow_constrained_rl]:
    ...
    def flow_constrained_rl4_loss(self, flow, behavior_flow, observations, actions, next_observations, rewards, dones, tau, multiple_actions = 10):
        loss_info = {}
    ...
    def dist_flow_constrained_rl_loss(self, flow, behavior_flow, observations, actions, next_observations, rewards, dones, tau):
        loss_info = {}
        v_loss, info = self.expectile_v_loss(
            tau=tau, observations=observations, actions=actions
        )
        ...
        with torch.no_grad():
            for t in np.linspace(time_start, time_end, steps):
                ...
                behavior_u_dist, behavior_u, behavior_u_log_sigma = behavior_flow(x=torch.cat([next_observations, x], dim=-1), t=t_tensor)
                v_dist, v, v_log_sigma = flow(x=torch.cat([next_observations, x], dim=-1), t=t_tensor)
                # divergence = 2*(behavior_u_log_sigma - v_log_sigma) + ((2*v_log_sigma).exp() + (v - behavior_u).pow(2)) / (2*behavior_u_log_sigma.exp().pow(2)) - 0.5
                divergence = torch.norm(v - behavior_u, dim=-1, keepdim=True)
    ...
    def flow_constrained_rl_loss(self, flow, behavior_flow, observations, actions, next_observations, rewards, dones, tau):
        num_samples = len(observations)
        x = torch.randn(num_samples, self.argus.action_dim, device=self.argus.device).clamp(-self.argus.x_t_clip_value, self.argus.x_t_clip_value)
        dt = 1.0 / self.argus.flow_step
        multi_x = [x]
