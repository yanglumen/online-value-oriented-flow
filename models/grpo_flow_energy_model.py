...
class grpo_iql_critic(nn.Module):
    ...
    def loss(self, flow, behavior_flow, observations, actions, next_observations, rewards, dones, tau):
        loss_info = {}
        v_loss, info = self.expectile_v_loss(
            tau=tau, observations=observations, actions=actions
        )
        ...
        for t in np.linspace(time_start, time_end, steps):
            ...
            with torch.no_grad():
                _, flow_u, _ = flow(torch.cat([multiple_next_observation, x], dim=-1), t_tensor)
                behavior_u = behavior_flow(x=torch.cat([multiple_next_observation, x], dim=-1), t=t_tensor)
            ...
            x = x.clamp(-self.argus.x_t_clip_value, self.argus.x_t_clip_value)
            step_index += 1
        min_Q = self.get_scaled_q(obs=multiple_next_observation, act=x, scale=self.argus.energy_scale)
        flow_v_loss = 0
        for step_i in range(len(multi_divergence)):
            target_flow_V = - self.argus.divergence_coef * torch.sum(torch.cat(multi_divergence[step_i:], dim=-1), dim=-1, keepdim=True) + min_Q.detach()
            flow_V = self.flow_v(torch.cat(
                [multi_x[step_i], actions.unsqueeze(1).repeat(1, self.argus.grpo_group_size, 1), multiple_next_observation], dim=-1))
            flow_v_loss += F.mse_loss(flow_V, target_flow_V.detach()) / len(multi_divergence)
        loss_info.update(
            {
