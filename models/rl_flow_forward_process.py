...
def sample_weighted_interpolated_points(
        argus, observations, actions, energy, beta, energy_model, weighted_samples_type, time_offset=0, x_0=None, clip_value=None):
    ...
    if weighted_samples_type == WeightedSamplesType.linear_interpolation:
        ...
        x_1 = actions
        if len(x_1.shape) == 3:
            t = t.unsqueeze(1).repeat(1, int(x_1.shape[1]), 1)
        x_0 = torch.randn_like(x_1, device=actions.device)
        if clip_value is not None:
            x_0 = torch.clip(x_0, -clip_value, clip_value)
        x_t = (1 - t) * x_0 + t * x_1
        dx_dt = x_1 - x_0
        weights = torch.ones_like(t, device=actions.device)
        return x_t, t, dx_dt, weights
    if weighted_samples_type == WeightedSamplesType.specific_start_end:
        t = torch.rand(len(actions), 1, device=actions.device)
        x_1 = actions
        x_t = (1 - t) * x_0 + t * x_1
        dx_dt = x_1 - x_0
        weights = torch.ones_like(t, device=actions.device)
        return torch.cat([observations, x_t], dim=-1), t, dx_dt, weights
    if weighted_samples_type == WeightedSamplesType.discrete_flow_step:
        dt = 1 / argus.flow_step
        t = torch.randint(argus.flow_step, size=(len(actions), 1), device=actions.device)
        t = t * dt
        x_1 = actions
        x_t = (1 - t) * x_0 + t * x_1
        dx_dt = x_1 - x_0
        weights = torch.ones_like(t, device=actions.device)
        return x_t, t, dx_dt, weights
    if weighted_samples_type == WeightedSamplesType.same_t_step:
        t = np.random.uniform(low=0.0, high=1.0-1e-3)
        t_tensor = torch.full((len(actions), 1), t, dtype=torch.float32, device=actions.device)
        x_1 = actions
        x_0 = torch.randn_like(x_1, device=actions.device)
        if clip_value is not None:
            x_0 = torch.clip(x_0, -clip_value, clip_value)
        ...
        dx_dt = x_1 - x_0
        weights = torch.ones_like(t_tensor, device=actions.device)
        return x_t, t, dx_dt, weights
    if weighted_samples_type == WeightedSamplesType.multiple_linear_interpolation:
        t = torch.rand(len(actions), 1, device=actions.device)
        x_1 = actions
        if len(x_1.shape) == 3:
            t = t.unsqueeze(1).repeat(1, int(x_1.shape[1]), 1)
        ...
        weights = torch.ones_like(t, device=actions.device)
        return x_t, t, dx_dt, weights
    if weighted_samples_type == WeightedSamplesType.adaptive_step_interpolation:
        t = torch.rand(len(actions), 1, device=actions.device)
        x_1 = actions
        if len(x_1.shape) == 3:
            t = t.unsqueeze(1).repeat(1, int(x_1.shape[1]), 1)
...
def sample_value_interpolated_points(
        argus, value, weighted_samples_type, time_offset=0, x_0=None, clip_value=None):
    if weighted_samples_type == WeightedSamplesType.linear_interpolation:
        t = torch.rand(len(value), 1, device=value.device)
        x_1 = value
        if len(x_1.shape) == 3:
            t = t.unsqueeze(1).repeat(1, int(x_1.shape[1]), 1)
        x_0 = torch.randn_like(x_1, device=value.device)
        if clip_value is not None:
            x_0 = torch.clip(x_0, -clip_value, clip_value)
        x_t = (1 - t) * x_0 + t * x_1
        dx_dt = x_1 - x_0
        weights = torch.ones_like(t, device=value.device)
        return x_t, t, dx_dt, weights
