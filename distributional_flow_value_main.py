...
from models.flow_model import FlowMatchingNet, LargeFlowMatchingNet, FlowValueNet
from trainer.distributional_flow_value_trainer import guided_flow_trainer
...
def import_parameters(var_kwargs=None, mode="distributional_flow_value_parameters"):
    print(colored(f"Loading {mode} parameter ......", color="green"))
    if mode == "distributional_flow_value_parameters":
        hyperparameters = distributional_flow_value_parameters
...
def train(**var_kwargs):
    argus = dict2obj(import_parameters(var_kwargs, mode="distributional_flow_value_parameters"))
    ...
    train_flow = FlowModel(
        argus=argus, input_dim=argus.observation_dim + argus.action_dim, output_dim=argus.action_dim)
    dist_flow_value = FlowValueNet(
        argus=argus, input_dim=argus.observation_dim + 1, output_dim=1)
    ...
    trainer = guided_flow_trainer(
        argus=argus, train_flow=train_flow, behavior_flow=behavior_flow, dist_flow_value=dist_flow_value, energy_model=energy_model, dataset=dataset)
