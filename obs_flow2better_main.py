...
from trainer.obs_flow2better_model_trainer import guided_flow_trainer
...
def train(**var_kwargs):
    ...
    inverse_dynamics_model = inverse_dynamics(
        input_dim=argus.observation_dim+argus.observation_dim, output_dim=argus.action_dim)
    flow_model = FlowMatchingNet(
        argus=argus, input_dim=argus.observation_dim+argus.observation_dim, output_dim=argus.observation_dim)
    ...
    trainer = guided_flow_trainer(
        argus=argus, model=flow_model, inverse_dynamic=inverse_dynamics_model, energy_model=energy_model, dataset=dataset)
