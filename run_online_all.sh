...
run_experiment() {
  ...
  local label="$2"
  local seed="$3"
  local behavior_only="$4"

  echo "============================================================"
  echo "Running rl_mode=${rl_mode} label=${label} seed=${seed} behavior_only=${behavior_only}"
  echo "============================================================"
...
run_experiment "adv_rl" "behavior_only_lr5e5_recentbuf" "$BASELINE_SEED" "True"

for seed in "${ADV_SEEDS[@]}"; do
  run_experiment "adv_rl" "adv_rl_lr5e5_bn_recentbuf" "$seed" "False"
done
