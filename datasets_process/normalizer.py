...
class DatasetNormalizer:
    ...
    def __init__(self, dataset, normalizer, path_lengths=None):
        ...
        for key, val in dataset.items():
            try:
                self.normalizers[key] = self.check_std_zero(normalizer(val))
            except:
                print(f'[ utils/normalization ] Skipping {key} | {normalizer}')
            # key: normalizer(val)
            # for key, val in dataset.items()

    def check_std_zero(self, normalizer):
        for i in range(len(normalizer.stds)):
            if np.abs(normalizer.stds[i]) < 1e-8:
                normalizer.stds[i] = 1.
        return normalizer
    ...
    def normalize2tensor(self, x, key):
        return self.normalizers[key].normalize2tensor(x)

    def unnormalize2tensor(self, x, key):
        return self.normalizers[key].unnormalize2tensor(x)

class OnlineDatasetNormalizer:
    ...
    def __init__(self, dataset, normalizer):
        ...
        self.normalizers = {}
        for key, val in dataset.normalize_keys.items():
            try:
                self.normalizers[key] = normalizer(dim=val)
            except:
                print(f'[ utils/normalization ] Skipping {key} | {normalizer}')
            # key: normalizer(val)
            # for key, val in dataset.items()

    def calculate_normalize_parameters(self, data):
        for normalizer_key in list(data.normalize_keys.keys()):
            self.normalizers[normalizer_key].calculate_normalize_parameters(data=np.vstack(data._dict[normalizer_key]))
...
def flatten(dataset, path_lengths):
    ...
    for key, xs in dataset.items():
        ...
        flattened[key] = [np.reshape(each_path, [-1, np.shape(each_path)[-1]]) for each_path in xs]
        flattened[key] = np.vstack(flattened[key])
    return flattened
...
class Normalizer:
    ...
    def __init__(self, X):
        ...
        self.mins2tensor = torch.tensor(self.mins)
        self.maxs2tensor = torch.tensor(self.maxs)
    ...
    def normalize2tensor(self, *args, **kwargs):
        raise NotImplementedError()

    def unnormalize2tensor(self, *args, **kwargs):
        raise NotImplementedError()
...
class GaussianNormalizer(Normalizer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.z = 1
        self.means2tensor = torch.tensor(self.means)
        self.stds2tensor = torch.tensor(self.stds)
    ...
    def normalize2tensor(self, x):
        # self.means2tensor = self.means2tensor.to(x.device)
        # self.stds2tensor = self.stds2tensor.to(x.device)
        return (x - self.means2tensor.to(x.device)) / self.stds2tensor.to(x.device)

    def unnormalize2tensor(self, x):
        # self.means2tensor = self.means2tensor.to(x.device)
        # self.stds2tensor = self.stds2tensor.to(x.device)
        return x * self.stds2tensor.to(x.device) + self.means2tensor.to(x.device)

class OnlineGaussianNormalizer(OnlineNormalizer):
    '''
        normalizes to zero mean and unit variance
    '''
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.means = np.zeros(self.dim)
        self.stds = np.zeros(self.dim)
        self.z = 1
        self.means2tensor = torch.tensor(self.means)
        self.stds2tensor = torch.tensor(self.stds)
    ...
    def calculate_normalize_parameters(self, data):
        ...
        self.stds[np.abs(self.stds) < 1e-8] = 1.0
        self.means2tensor = torch.tensor(self.means)
        self.stds2tensor = torch.tensor(self.stds)
...
class LimitsNormalizer(Normalizer):
    ...
    def unnormalize(self, x, eps=1e-4):
        ...
        if x.max() > 1 + eps or x.min() < -1 - eps:
            # print(f'[ datasets_process/mujoco ] Warning: sample out of range | ({x.min():.4f}, {x.max():.4f})')
            x = np.clip(x, -1, 1)
...
class CDFNormalizer1d:
    ...
    def __init__(self, X):
        ...
        if self.X.max() == self.X.min():
            ...
            # we want to contrib the mapping function from quantiles to cdf_prob, i.e., if quantiles = [1, 2, 3, 4], cumprob=[0.1, 0.3, 0.6, 1.0],then self.fn([1.5, 2.5, 3.5]) will be [0.2 0.4 0.8]
    ...
    def normalize(self, x):
        ...
        # calculate the position of x in the whole dataset
        x = np.clip(x, self.xmin, self.xmax)
        ## [ 0, 1 ]   calculate the cdf prob according to the cdf_prob interpolate function
        y = self.fn(x)
        ## [ -1, 1 ]  transfer the y into [-1, 1]
        y = 2 * y - 1
        return y
